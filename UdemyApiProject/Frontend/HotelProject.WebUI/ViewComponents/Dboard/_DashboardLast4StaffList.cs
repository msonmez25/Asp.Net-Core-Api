...
namespace HotelProject.WebUI.ViewComponents.Dboard
{
    public class _DashboardLast4StaffList : ViewComponent
    {
        ...
        public async Task<IViewComponentResult> InvokeAsync()
        {
            ...
            var responseMessage = await client.GetAsync("http://localhost:5248/api/Staff/Last4Staff");
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
                var values = JsonConvert.DeserializeObject<List<ViewLast4StaffDto>>(jsonData);
                return View(values);
            }
            return View();
        }
